%%% @doc 
%%% BLOCKTYPE
%%% Output timestamp of last time block was executed
%%% DESCRIPTION
%%% Block output is the last time the block was executed
%%% Format: "year/mon/day hr:min:sec.uSecs" 
%%% Example: "2018/04/01 20:15:34.589234"
%%% LINKS
%%% @end 

-module(lblx_timestamp).  
...
default_inputs() -> 
  attrib_utils:merge_attribute_lists(
    block_common:inputs(),
    [
      {input, {empty, {empty}}}  %| any | empty | N/A |
    ]). 
...
execute({Config, Inputs, Outputs, Private}, _ExecMethod) ->
  ...
  % Translate to human readable string
  LastExecuted = lists:flatten(
                  io_lib:format("~4w/~2..0w/~2..0w ~2w:~2..0w:~2..0w.~6..0w", 
                               [Year, Month, Day, Hour, Minute, Second, Micro])),

  Outputs1 = output_utils:set_value_normal(Outputs, LastExecuted),
