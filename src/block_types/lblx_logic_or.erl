...
-export([or_inputs/5]). % Special for NOR block type

groups() -> [logic].

description() -> "OR of All of the Binary Inputs".
...
%
% OR all of the binary input values together
%
or_inputs(Inputs, Index, NumOfInputs, IgnoreNulls, Values) when Index =< NumOfInputs ->

  case input_utils:get_boolean(Inputs, {inputs, Index}) of
     
    % Build a list of input values. Value is true, false, or null.
    {ok, Value} -> or_inputs(Inputs, Index+1, NumOfInputs, IgnoreNulls, [Value | Values]);
...
or_inputs(_Inputs, _Index, _NumOfInputs, IgnoreNulls, Values) -> 
  case IgnoreNulls of
    true -> % Ignoring null input values
      case lists:member(true, Values) of
        true  -> % At least one input was true, output value is true
          {ok, true};
        false -> 
    ...
    false -> % Not ignoring null input values
      case lists:member(null, Values) of
        ...
        false -> % No inputs were null
          % If at least one input was true, then output value is true, otherwise false.
          case lists:member(true, Values) of
            true  -> {ok, true};
            false -> {ok, false}
          end
...
test_sets() ->
  [
    ...
    {[{{inputs, 1}, true}, {{inputs, 2}, false}, {{inputs, 3}, false}, {{inputs, 4}, false}, {{inputs, 5}, false}], [{status, normal}, {value, true}]},
...
test_sets_ignore_nulls() ->
  [
    ...
    {[{{inputs, 1}, false}, {{inputs, 2}, false}, {{inputs, 3}, null}, {{inputs, 4}, false}, {{inputs, 5}, true}], [{status, normal}, {value, true}]},
    {[{{inputs, 1}, true}, {{inputs, 2}, false}, {{inputs, 3}, false}, {{inputs, 4}, false}, {{inputs, 5}, false}], [{status, normal}, {value, true}]},
