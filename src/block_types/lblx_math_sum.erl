%%% @doc 
%%% Block Type: Sum all input values
%%% Description:  Set the block output value to the sum of all input values
%%%               
%%% @end 

-module(lblx_math_sum).  
...
execute({Config, Inputs, Outputs, Private}, _ExecMethod) ->
  ...
  case sum_inputs(Inputs, 1, NumOfInputs, IgnoreNulls, 0, 0) of
...
%
% Sum all of the numerical input values together
%
sum_inputs(Inputs, Index, NumOfInputs, IgnoreNulls, ActiveInputs, Sum) when Index =< NumOfInputs ->

  case input_utils:get_number(Inputs, {inputs, Index}) of

    {ok, null} ->
      sum_inputs(Inputs, Index+1, NumOfInputs, IgnoreNulls, ActiveInputs, Sum);
     
    % Sum the input values. Value is a number.
    {ok, Value} -> 
      sum_inputs(Inputs, Index+1, NumOfInputs, IgnoreNulls, (ActiveInputs + 1), (Value + Sum));
...
sum_inputs(_Inputs, _Index, NumOfInputs, IgnoreNulls, ActiveInputs, Sum) -> 
  case IgnoreNulls of
...
test_sets_ignore_nulls() ->
  [
    ...
    {[{{inputs, 1}, 11.1}, {{inputs, 2}, 22.2}, {{inputs, 3}, 33.3}, {{inputs, 4}, 44.4}, {{inputs, 5}, 55.5}], [{status, normal}, {value, 166.5}]},
    {[{{inputs, 1}, 11.1}, {{inputs, 2}, 22}, {{inputs, 3}, 33}, {{inputs, 4}, 44}, {{inputs, 5}, 55}], [{status, normal}, {value, 165.1}]}
 ].
