...
%
listen_start() ->
  m_logger:info(starting_node_watcher),
...
listen_loop() ->
  receive 
    {nodeup, Node} ->
      m_logger:info(node_has_connected, [Node]),
    ...
    {nodedown, Node} ->
      m_logger:info(node_has_disconnected, [Node]);

    Unexpected ->
      m_logger:warning(node_watcher_received_unexpected_msg, [Unexpected])
